...
jQuery(function ($) {
  ...
  if (modal) {
    // モーダルを表示する関数
    var openModal = function openModal(imageSrc, caption) {
      var modalImage = modal.querySelector(".modal__img");
      if (modalImage) {
        modalImage.innerHTML = ""; // 既存の画像をクリア

        // 画像を生成してモーダル内に表示
        modalImage.innerHTML = "<img src=\"".concat(imageSrc, "\" alt=\"").concat(caption, "\" />");
        modal.classList.add("show");
        document.body.style.overflow = "hidden"; // スクロールバーを非表示
      }
    }; // モーダルトリガーのイベントリスナーを設定

    var modalTriggerElements = document.querySelectorAll(".js-modal-trigger");
  ...
  // ------------------
  // .blog-card要素をすべて取得
  var blogCards = document.querySelectorAll(".js-hover-scale");

  // 各.blog-card要素に対してループ処理
  blogCards.forEach(function (blogCard) {
    // img要素を取得
    var imgElement = blogCard.querySelector("img");
  ...
  document.querySelectorAll(".js-collapse").forEach(function (button) {
